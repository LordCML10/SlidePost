...
export default function LibraryPage() {
  ...
  async function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // that mix aspect ratios OR pixel dimensions, and on 4:4:4 / ICC-tagged
      // images. Forcing one fixed output size sidesteps all of it: every image in
      // every slideshow is byte-shape-identical (same dims, same 9:16 ratio),
      // 4:2:0 (canvas at 0.8 quality), and free of ICC/EXIF (canvas strips them).
      // (Native-resolution crop was tried and still failed — TikTok needs uniform
      // dimensions, not just uniform ratio. Do not remove the fixed size.)
